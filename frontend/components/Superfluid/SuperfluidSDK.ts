...
export const createFlow = async (sender: string, receiver: string, flowRate: string) => {
  //@ts-ignore
...
export const createOrRevokePermission = async (flowRate: string, operator: string, permission: number) => {
  // @ts-ignore
  const provider = new ethers.providers.Web3Provider(window.ethereum);
  const signer = provider.getSigner();
